/*
╔══════════════════════════════════════════════════════════════════════════════╗
║  FILE: crm_client_logic.js – Client List, Ledger, and CRM Functions         ║
║         (Added loyalty points display, points adjustment)                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
*/
        ...
        body.innerHTML += `
            <button class="list-item-btn" onclick="viewClientLedger('${c.id}', this)">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <span style="font-size:0.75rem; color:${isDebt ? 'var(--col-danger)' : 'var(--col-success)'}">${appSettings.property.currency} ${c.balance.toFixed(0)}</span>
                </div>
...
function renderCrmDashboard() {
    ...
    app.clients.forEach(c => totalReceivables += (c.balance > 0 ? c.balance : 0));

    const topDebtors = app.clients.filter(c => c.balance > 0).sort((a, b) => b.balance - a.balance).slice(0, 5);

    let topDebtorsHtml = '';
    if (topDebtors.length === 0) {
        topDebtorsHtml = '<p style="text-align:center; color:var(--text-secondary); font-style:italic;">No outstanding debts currently.</p>';
    } else {
        topDebtors.forEach((c, idx) => {
            topDebtorsHtml += `
                <div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid rgba(0,0,0,0.05);">
                    ...
                    <span style="color:var(--col-danger); font-weight:700;">${appSettings.property.currency} ${c.balance.toFixed(0)}</span>
                </div>
            `;
        });
    }
    ...
    b.innerHTML = `
        <h3 style="margin-bottom:15px; color:var(--col-primary);">CRM Overview</h3>

        <div class="pay-info-box" style="background:var(--bg-app); padding:15px; border-radius:15px; box-shadow:var(--neumorph-out-sm); margin-bottom:20px;">
            <div class="pay-info-label">Total Pending Receivables</div>
            <div class="pay-info-val" style="font-size:1.8rem; font-weight:800; color:var(--col-danger); text-align:center; font-family:var(--ui-font-dash-num-family, ${uiFont.dashNumFamily}); font-style:var(--ui-font-dash-num-style, ${uiFont.dashNumStyle});">${appSettings.property.currency} ${totalReceivables.toFixed(0)}</div>
        </div>

        <h4 style="font-size:0.95rem; color:var(--col-secondary); margin-bottom:10px; border-bottom:1px solid rgba(0,0,0,0.1); padding-bottom:5px;">Top Debtors</h4>
        <div style="background:var(--bg-app); padding:15px; border-radius:15px; box-shadow:var(--neumorph-in-sm);">
            ${topDebtorsHtml}
        </div>

        <div style="margin-top:30px; text-align:center;">
            <p style="font-size:0.8rem; color:var(--text-secondary);">Select a client from the left panel to view their detailed ledger and profile.</p>
        </div>
    `;
    document.querySelectorAll('#cl-detail-body .pay-info-val').forEach(el => {
        el.style.fontFamily = `var(--ui-font-dash-num-family, ${uiFont.dashNumFamily})`;
        el.style.fontStyle = `var(--ui-font-dash-num-style, ${uiFont.dashNumStyle})`;
    });
}
...
function newClientForm() {
    ...
    b.innerHTML = `
        ...
        <div class="input-group"><label>Name *</label><input type="text" id="nc-name" placeholder="John Doe" class="modern-input" data-edit-id=""></div>
        ...
        <div class="input-group"><label>Monthly Credit Limit (${appSettings.property.currency})</label><input type="number" id="nc-limit" placeholder="0" value="0" class="modern-input"></div>
...
function viewClientLedger(id, elem) {
    ...
    let h = `
        ...
        <div style="background:var(--bg-app); padding:15px; border-radius:15px; margin-bottom:20px;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-weight:700; font-size:1rem;">Loyalty Points</span>
                <span style="font-size:1.5rem; font-weight:800; color:var(--col-primary);">${c.loyaltyPoints || 0}</span>
            </div>
            <div style="margin-top:10px;">
                <button class="btn-modern" style="padding:5px 10px;" onclick="adjustClientPoints('${c.id}')">Adjust Points</button>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:10px;">
            <div class="pay-info-box" style="background:var(--bg-app); padding:10px; border-radius:12px; box-shadow:var(--neumorph-in); text-align:center;">
                <div class="pay-info-label">Balance Due</div>
                <div class="pay-info-val" style="font-size:1.1rem; font-weight:800; color:${c.balance > 0 ? 'var(--col-danger)' : 'var(--col-success)'}; font-family:var(--ui-font-dash-num-family, ${uiFont.dashNumFamily}); font-style:var(--ui-font-dash-num-style, ${uiFont.dashNumStyle});">${appSettings.property.currency} ${c.balance.toFixed(0)}</div>
            </div>
            <div class="pay-info-box" style="background:var(--bg-app); padding:10px; border-radius:12px; box-shadow:var(--neumorph-in); text-align:center;">
                <div class="pay-info-label">Total Purchased</div>
                <div class="pay-info-val" style="font-size:1.1rem; font-weight:800; color:var(--col-primary); font-family:var(--ui-font-dash-num-family, ${uiFont.dashNumFamily}); font-style:var(--ui-font-dash-num-style, ${uiFont.dashNumStyle});">${appSettings.property.currency} ${c.totalPurchasing.toFixed(0)}</div>
            </div>
        </div>

        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; margin-bottom:20px;">
            <div class="pay-info-box" style="background:var(--bg-app); padding:10px; border-radius:12px; box-shadow:var(--neumorph-out-sm); text-align:center;">
                <div class="pay-info-label">Orders</div>
                <div class="pay-info-val" style="font-size:1rem; font-weight:800; color:var(--text-primary); font-family:var(--ui-font-dash-num-family, ${uiFont.dashNumFamily}); font-style:var(--ui-font-dash-num-style, ${uiFont.dashNumStyle});">${c.totalOrders}</div>
            </div>
            <div class="pay-info-box" style="background:var(--bg-app); padding:10px; border-radius:12px; box-shadow:var(--neumorph-out-sm); text-align:center;">
                <div class="pay-info-label">Discount</div>
                <div class="pay-info-val" style="font-size:1rem; font-weight:800; color:var(--col-danger); font-family:var(--ui-font-dash-num-family, ${uiFont.dashNumFamily}); font-style:var(--ui-font-dash-num-style, ${uiFont.dashNumStyle});">${c.totalDiscount}</div>
            </div>
            <div class="pay-info-box" style="background:var(--bg-app); padding:10px; border-radius:12px; box-shadow:var(--neumorph-out-sm); text-align:center;">
                <div class="pay-info-label">Limit</div>
                <div class="pay-info-val" style="font-size:1rem; font-weight:800; color:var(--text-primary); font-family:var(--ui-font-dash-num-family, ${uiFont.dashNumFamily}); font-style:var(--ui-font-dash-num-style, ${uiFont.dashNumStyle});">${c.limit > 0 ? c.limit : 'None'}</div>
            </div>
        </div>
    ...
    <h4 style="font-size:0.85rem; color:var(--col-secondary); margin:20px 0 10px; text-transform:uppercase;">Points History</h4>
    <div style="overflow-x:auto;">
        <table class="data-table">
            <thead><tr><th>Date</th><th>Change</th><th>Reason</th><th>Order #</th></tr></thead>
            <tbody>
    `;

    if (!c.pointsHistory || c.pointsHistory.length === 0) {
        h += `<tr><td colspan="4" style="text-align:center; color:var(--text-secondary); font-style:italic;">No points history</td></tr>`;
    } else {
        c.pointsHistory.slice().reverse().forEach(ph => {
            h += `
                <tr>
                    <td>${ph.date}</td>
                    <td style="color:${ph.change > 0 ? 'var(--col-success)' : 'var(--col-danger)'};">${ph.change > 0 ? '+' : ''}${ph.change}</td>
                    <td>${ph.reason}</td>
                    <td>${ph.orderId || '--'}</td>
                </tr>
            `;
...
function adjustClientPoints(clientId) {
    const c = app.clients.find(x => x.id === clientId);
    openCustomPrompt(`Adjust points for ${c.name}`, null, 0, 0, 'both', (result) => {
        if (result.price !== null) {
            const change = parseInt(result.price) || 0;
            c.loyaltyPoints = (c.loyaltyPoints || 0) + change;
            if (!c.pointsHistory) c.pointsHistory = [];
...
function receiveClientPayment(id) {
    ...
    const c = app.clients.find(x => x.id === id);
    openCustomPrompt(`Receive Payment from ${c.name}`, null, c.balance > 0 ? c.balance : '', null, 'price', (result) => {
        if (result.price === null) return;
        let amt = parseFloat(result.price);
        ...
        localStorage.setItem('pos_clients', JSON.stringify(app.clients));
        showToast(`Received ${appSettings.property.currency} ${amt}`);
        renderClientsList();
        viewClientLedger(id);
        renderCrmDashboard();
    });
